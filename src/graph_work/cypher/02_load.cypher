...
// Stage 2-5 sections use apoc.load.csv inside a CALL so they NO-OP gracefully when
// a file is absent (Stage-1-only graphs still load cleanly). APOC required.
...
// ---------------------------------------------------------------------------
CALL apoc.load.csv('file:///nodes_npi_enriched.csv') YIELD map AS r
MATCH (n:NPI {npi: r.npi})
...
CALL apoc.load.csv('file:///nodes_address.csv') YIELD map AS r
MERGE (:Address {address_id: r.address_id})
RETURN count(*) AS addresses;

CALL apoc.load.csv('file:///nodes_phone.csv') YIELD map AS r
MERGE (:Phone {digits: r.digits})
RETURN count(*) AS phones;

CALL apoc.load.csv('file:///nodes_person.csv') YIELD map AS r
...
CALL apoc.load.csv('file:///edges_npi_address.csv') YIELD map AS r
...
CALL apoc.load.csv('file:///edges_npi_phone.csv') YIELD map AS r
...
CALL apoc.load.csv('file:///edges_npi_person.csv') YIELD map AS r
...
// POSSIBLY_SAME_AS between persons sharing a name_base but distinct tiebreakers
// (NEVER auto-merged — a false merge fabricates a fraud link).
MATCH (p1:Person), (p2:Person)
WHERE p1.name_base = p2.name_base AND p1.person_id < p2.person_id
MERGE (p1)-[:POSSIBLY_SAME_AS]-(p2);

// ---------------------------------------------------------------------------
// STAGE 3 — ownership (optional). Owner may be Person or Org.
// ---------------------------------------------------------------------------
CALL apoc.load.csv('file:///edges_owns.csv') YIELD map AS r
MATCH (n:NPI {npi: r.npi})
CALL apoc.do.when(
  r.owner_type = 'Org',
...
// ---------------------------------------------------------------------------
CALL apoc.load.csv('file:///edges_associated_with.csv') YIELD map AS r
MATCH (n:NPI {npi: r.npi})
...
// ---------------------------------------------------------------------------
CALL apoc.load.csv('file:///flags_leie_npi.csv') YIELD map AS r
MATCH (n:NPI {npi: r.npi})
SET n.leie_excluded = true, n.leie_excltype = r.EXCLTYPE, n.leie_excldate = r.EXCLDATE
RETURN count(*) AS leie_npi_flags;

CALL apoc.load.csv('file:///flags_leie_person.csv') YIELD map AS r
